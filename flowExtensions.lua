...
local function computeNorm(...)
   ...
   local x_squared = torch.Tensor()
   flow_x = flow_x:typeAs(x_squared)
   flow_y = flow_y:typeAs(flow_norm)
   x_squared:resizeAs(flow_x):copy(flow_x):cmul(flow_x)
...
local function computeAngle(...)
   ...
   flow_x = flow_x:typeAs(flow_angle)
   flow_y = flow_y:typeAs(flow_angle)
